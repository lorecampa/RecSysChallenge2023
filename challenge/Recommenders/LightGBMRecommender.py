...
class LightGBMRecommender(BaseRecommender):
    ...
    def __init__(self, URM_train, URM_val, load_model_path=None):
        super(LightGBMRecommender, self).__init__(URM_train)
    ...
    def fit(self,
        ...
        self.features[categorical_columns] = self.features[categorical_columns].astype('category')
        ...
        self.model = lgb.LGBMRanker(
            ...
            objective=objective,
            metric=metric,
            categorical_feature=f'name:{",".join(categorical_columns)}'
        )
    ...
    def _compute_features(self, URM, recs, cutoff, load=False):
        ...
        if (not load):
            ...
            # generators_name = ['SLIMElasticNetRecommenderCrossValRecall40', 'RP3betaRecommenderCrossValRecall40']
            ...
            # df.to_pickle('df_lgbm.pkl')
        else:
            df = pd.read_pickle('df_lgbm.pkl')
        return df
    ...
    def normalize_recs_scores(self, df: pd.DataFrame, other_recs):
        ...
        df[to_normalize] = StandardScaler().fit_transform(df[to_normalize].values)
    ...
    def generate_dissimilarity_features(self, df: pd.DataFrame, other_recs):
        ...
        for pair in tqdm(model_pairs, desc='Generating user based dissimilarity features'):
            pair1, pair2 = pair
            df[f'{pair[0]}_{pair[1]}_abs_diff'] = (df[pair1] - df[pair2]).abs()
        return df
    ...
    def plot_importance(self, max_num_features=30):
        lgb.plot_importance(self.model, importance_type='split', title='Weight (Frequency)', max_num_features=max_num_features)
        plt.show()
